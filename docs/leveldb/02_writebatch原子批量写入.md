...
了解了 WriteBatch 的实现细节后，让我们退一步思考：LevelDB 的设计者**为什么**要这样设计？这些看似简单的选择背后，隐藏着深思熟虑的工程权衡。

### 为什么用单一 `rep_` 字符串而不是结构体数组？

你可能会想：把操作存成一个 `vector<Operation>` 不是更直观吗？为什么要用一个"手工编码"的字节串 `rep_`？

| 方案 | 优点 | 缺点 |
|------|------|------|
| `vector<Operation>` | 类型安全，易操作 | 序列化开销大，内存碎片 |
| 单一 `string rep_` | 零拷贝写入WAL，紧凑 | 需手动编解码 |

LevelDB 选择 `rep_` 的关键原因：WriteBatch 的内容需要**直接写入 WAL 日志**。回顾上面阶段二的代码：
...
`Contents()` 返回的就是 `rep_`——直接把这个字节串交给日志，**零拷贝，零序列化**。

如果用 `vector<Operation>`，每次写日志都需要先把结构体数组序列化成字节流，这是一次额外的 O(n) 操作和一次额外的内存分配。而用 `rep_` 则是**构建即序列化**——`Put()` 和 `Delete()` 在添加操作的同时就完成了编码，写日志时直接拿来用。

对于一个每秒可能处理数十万次写入的数据库来说，省掉这一步序列化意义重大。

### 为什么用 Handler 模式回放，而不是直接解析？

回放 WriteBatch 时，LevelDB 没有让调用者自己去解析 `rep_` 中的字节，而是提供了 `Iterate` + `Handler` 的模式。为什么要多这一层抽象？

`Iterate` 方法负责**解析**字节流，`Handler` 接口负责**消费**解析出的操作。这让**解析逻辑和消费逻辑完全解耦**。

同一个 `Iterate` 可以服务于不同的目的：

- **写入 MemTable**：使用 `MemTableInserter` 作为 Handler
- **计算操作大小**：可以实现一个计数 Handler
- **调试打印**：可以实现一个打印 Handler
- **合并 batch**：可以实现一个转发 Handler

无需为每种用途重复编写解析代码。这是经典的**访问者模式（Visitor Pattern）**——数据结构（WriteBatch）不变，通过不同的访问者（Handler）实现不同的行为。

这种设计还有一个好处：如果未来编码格式发生变化，只需要修改 `Iterate` 一处，所有的 Handler 都不受影响。
