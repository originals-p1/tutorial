...
void DBIter::Seek(const Slice& target) {
  direction_ = kForward;
  saved_key_.clear();
  // 构造内部键：(target, 快照序列号, kTypeValue)
  // kValueTypeForSeek 实际上就是 kTypeValue，
  // 配合序列号确保定位到 ≤ 该序列号的最新版本
...
先构造一个**内部格式的键**（用户键 + 快照序列号 + `kTypeValue`），交给内部迭代器定位，然后用 `FindNextUserEntry` 找到第一个有效记录。由于内部键的排序规则是 `(user_key 升序, sequence 降序, type 降序)`，用最大的序列号和类型值构造查找键，可以精确定位到该用户键在快照范围内的最新版本。
